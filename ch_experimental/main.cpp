...
int main() {
    // should be input
    std::string path="paris-0001000.instance";
    CS cs = CS::MIN;  // c = 0,1,2 -> random, min, max
    ...
    // for each edge we will:
    Points closest_points;              // finding the closest point
    std::vector<NUM> areas;             // if it is visible
    std::vector<bool> visibility;       // area of created triangle (dent)
    std::vector<int> inner_points_i;    // index of closest point at inner_points

    // initialise the above
    for(int i=0; i<poly_line.edges().size(); i++) 
        ch_to_poly_insert(i,
                          poly_line,   
                          inner_points,
                          closest_points, 
    ...
    for (int reps=0; reps<max_reps; reps++) {
        if(reps%50==0) std::cout<<reps<<std::endl;
        ...
        Point_2 new_point=closest_points[chosen_index];

        // insert new point in poly_line
        if (poly_line.vertices_begin()+chosen_index == poly_line.vertices_end()) 
            poly_line.push_back(new_point);
        else
            poly_line.insert(poly_line.vertices_begin()+chosen_index+1,new_point);

        // update area
        poly_area-=areas[chosen_index];

        // update inner_points, remove added point
        int chosen_inner_i=inner_points_i[chosen_index];
        for (int i=0; i<inner_points_i.size(); i++){
            if (inner_points_i[i]>=chosen_inner_i)
                inner_points_i[i]--;
        }
        inner_points.erase(inner_points.begin() + chosen_inner_i);

        // vis - increase vis_counter and save new poly line
        if(vis) vis_counter++;
        if(vis) save_points_to_file(poly_line,vis_poly_line+std::to_string(vis_counter));

        // prepare for next rep - skip if we are at last rep
        if (reps==max_reps-1) continue;
        // update closest_points, inner_points_i, areas, visibility
        // for new edge created by adding new_point - insertstd::cout<<"AAAAAAAAA\n";
        ch_to_poly_insert(chosen_index,
                          poly_line,   
                          inner_points,
                          closest_points, 
                          areas, 
        ...
        // for edges that previously had new_points as their closest one
        for(int i=0; i<poly_line.size(); i++) {
            // skip new edges
            if (closest_points[i] == new_point || i == chosen_index-1)
                ch_to_poly_update(i,
                                  poly_line,   
                                  inner_points,
                                  closest_points, 
